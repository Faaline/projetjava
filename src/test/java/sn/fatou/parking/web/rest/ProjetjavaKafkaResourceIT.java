...
import sn.fatou.parking.config.KafkaProperties;

class ProjetjavaKafkaResourceIT {
    ...
    void setup() {
        ...
        ProjetjavaKafkaResource kafkaResource = new ProjetjavaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/projetjava-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/projetjava-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
